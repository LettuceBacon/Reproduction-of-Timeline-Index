...
public class TemporalMinAggregator implements Aggregator{
    ...
    public TemporalMinAggregator(Class<?> type) {
        this.type = type;
        ...
        this.topKSet.insert(TypeUtil.getMaxOfType(type));
    }
    
    @Override
    public void collect(Object value, boolean flag) {
        if (flag == true) {
            if (topKSet.size() == 0 || cmpr.compare(value, topKSet.getMaxValue()) <= 0) { // 小于或等于topk集最大值
